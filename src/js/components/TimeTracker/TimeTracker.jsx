...
class TimeTracker extends React.Component {
    ...
    endTracker = () => {
        ...
        if (this.state.isStarted) {
            ...
            let date = new Date();
            let timeTrackerData = JSON.parse(localStorage.getItem('TimeTrackerData'));
            let currentSecond = date.getSeconds().toString().length !== 2 ? "0" + date.getSeconds().toString() : date.getSeconds().toString();
            let currentMinute = date.getMinutes().toString().length !== 2 ? "0" + date.getMinutes().toString() : date.getMinutes().toString();
            let currentHour = date.getHours().toString().length !== 2 ? "0" + date.getHours().toString() : date.getHours().toString();

            const currentTime = currentHour + " : " + currentMinute + " : " + currentSecond;

            const currentTotalDate = this.state.timer.hour + " : " + this.state.timer.minute + " : " + this.state.timer.second;
            let trackerDailyHistory = [...this.state.trackerDailyHistory];
            ...
            this.setState({
                ...
                isStarted: false,
                intervalState: null,
                trackerDailyHistory: trackerDailyHistory
            })
    ...
    render() {
        ...
        let timer = this.state.timer.hour + " : " + this.state.timer.minute + " : " + this.state.timer.second;
        const trackerDailyHistory = [...this.state.trackerDailyHistory];
        let trackerDailyHistoryItems = trackerDailyHistory.map((item, key) => {
